...
for pid,line,chute,times,depot in [(1,0,98,3,1),(1,1,98,2,1),(2,0,16,1,1),(0,0,98,2,1),(None,0,98,1,1),(4,2,98,2,1),(6,0,98,2,2),(999,0,98,4,1)]:
...
count=c.execute(cte+' SELECT COUNT(*) FROM valid_chute98',params).fetchone()[0]
assert len(rows)==count
assert not c.execute(cte+tail,dict(params,shiftStart='2026-09-18 15:00',shiftEnd='2026-09-19 03:00')).fetchall()
print('Chute 98 SQL checks passed: KPI count, repeated real-parcel passages, missing/unknown IDs excluded without duplicate parcel joins, depot and shift scope.')

summary_sql=s.split('public async Task<ConveyorQualityResponse>',1)[1].split('quality_summary AS (',1)[1].split('),\n            parcel_weights AS (',1)[0]
summary_count=c.execute(cte+', quality_summary AS ('+summary_sql+') SELECT chute_98 FROM quality_summary',params).fetchone()[0]
assert summary_count==len(rows)
