...
Le premier démarrage entraîne un modèle si aucun modèle valide n'existe. Ensuite, le renouvellement du samedi entraîne une nouvelle version; les autres jours chargent le dernier modèle valide et produisent seulement les sept prédictions du nouvel instantané. En cas d'échec d'entraînement ou de chargement, la prévision statistique continue d'être archivée et affichée. LightGBM utilise au plus deux fils d'exécution afin de préserver les ressources du tableau de bord.
...
« Prévu et réel » affiche les 30 dernières versions, filtrables, et compare uniquement les journées terminées de l'historique chargé. Les nouvelles versions v4 couvrent J à J+6; les anciennes versions conservent leur horizon J+1 à J+7. La prévision v4 du jour courant, créée à 6 h ou rattrapée avant la clôture de cette journée, est comparée au réel une fois la journée terminée. Les versions anciennes conservent leur critère de création avant le début du jour prévu. Le réel peut évoluer lors de corrections de la source; les prévisions ne changent pas. Pour une date passée sans archive, l'écran indique explicitement une reconstitution non archivée.
...
Depuis la version `weekday-annual-v4-today-cyber-monday`, le renouvellement national de 6 h commence le jour même (J à J+6). Les fichiers `yyyy-MM-dd-v4.json` préservent les fichiers v3 et leur horizon initial. Le test rétrospectif national utilise aussi J à J+6, toujours avec des observations strictement antérieures à la date de calcul. Le calendrier hebdomadaire des prévisions de livraison par secteur reste inchangé.
