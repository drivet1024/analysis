...
app.MapGet("/api/edi/history", async (string? date, EdiHistoryService data, CancellationToken cancellationToken) =>
{
    try
    {
...
app.MapGet("/api/edi/map", async (string? date, EdiMapService data, CancellationToken cancellationToken) =>
{
    ...
    catch (Exception ex) { return Results.Problem($"Carte des envois indisponible : {ex.Message}"); }
});
...
app.MapGet("/api/edi/depots/{depotId:int}/chutes", async (int depotId, string? date, EdiDepotChuteService data, CancellationToken cancellationToken) =>
{
    try
    {
        var today = CurrentOperationalDate(DateTime.Now);
        var selected = ResolveAnalysisDate(date, today);
        if (selected > today) return Results.BadRequest("La date ne peut pas être future.");
        var result = await data.GetAsync(selected, depotId, cancellationToken);
...
app.MapGet("/api/conveyor-chute16/parcels", async (string? date, string? depot, ConveyorDataService data) =>
{
    try
    {
        var selectedDepot = ConveyorCatalog.ResolveDepot(depot);
        return Results.Ok(await data.GetConveyorChute16Async(
            ResolveAnalysisDate(date, CurrentOperationalDate(DateTime.Now, selectedDepot)), selectedDepot));
    }
    catch (ArgumentException ex) { return Results.BadRequest(ex.Message); }
...
sealed class ConveyorDataService(DashboardConfig config, EdiForecastArchive forecastArchive)
{
    ...
    public async Task<ConveyorChute16Response> GetConveyorChute16Async(DateOnly date, DepotDefinition depot)
    {
        const string sql = ConveyorRecirculationCte + """
            , chute16 AS (
                SELECT * FROM scope
                WHERE chute=16 AND NOT ((parcel_id IS NULL OR parcel_id=0) AND COALESCE(camera_data,'') LIKE '?%')
            ), parcel_customers AS (
                SELECT p.PARCEL_ID parcel_id,MAX(NULLIF(p.CUSTOMER_ID,0)) customer_id
                FROM parcel p
        ...
        command.Parameters.AddWithValue("@hasFloor", depot.HasFloorConveyor);
        await using var reader = await command.ExecuteReaderAsync();
        var rows = new List<ConveyorChute16Row>();
        while (await reader.ReadAsync())
        {
