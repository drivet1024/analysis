...
sealed class EdiDepotService(DashboardConfig config, ConveyorDataService edi)
{
    ...
    public async Task<EdiDepotResponse> GetAsync(DateOnly date, CancellationToken cancellationToken)
    {
        ...
        if (cache.TryGetValue<EdiDepotResponse>(key, out var ready)) return ready!;
        await gate.WaitAsync(cancellationToken);
        try
        {
            if (cache.TryGetValue<EdiDepotResponse>(key, out var hit)) return hit!;
            ...
            await using var command = new MySqlCommand("""
                WITH postal_map AS (
                 ...
                 SELECT SHIPPING_ID,EXP_DATE,0 period,COUNT(*) parcels FROM parcel
                 WHERE INSERT_DATE>=@start AND INSERT_DATE<@end AND PARCEL_STATUS NOT IN (500,501)
                 GROUP BY SHIPPING_ID,EXP_DATE
                 UNION ALL
                 SELECT SHIPPING_ID,EXP_DATE,1 period,COUNT(*) parcels FROM parcel
                 ...
                 SELECT SHIPPING_ID,EXP_DATE,period,MAX(parcels) parcels,
                 CASE WHEN COUNT(DISTINCT depot_id)>1 THEN -2 ELSE MIN(depot_id) END depot_id
                 FROM candidates GROUP BY SHIPPING_ID,EXP_DATE,period
                )
                ...
                GROUP BY a.depot_id,d.DEPOTNAME ORDER BY parcels_today DESC,a.depot_id;
            ...
            var rows = new List<EdiDepotRow>();
            await using var reader = await command.ExecuteReaderAsync(cancellationToken);
            while (await reader.ReadAsync(cancellationToken)) rows.Add(new(reader.GetInt32("depot_id"), reader.GetString("depot_name"), reader.GetInt64("parcels_today"), reader.GetInt64("parcels_d7")));
            var result = new EdiDepotResponse(date, summary.Nowcast.AsOf, rows);
